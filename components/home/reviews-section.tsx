...
export function ReviewsSection() {
  const { data: reviews } = useFeaturedReviews(3)
    ...
    <section className="bg-secondary/50 py-12 md:py-16">
      <div className="container mx-auto px-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, index) => (
                      <Star
                        key={index}
                        className={`size-3 ${index < review.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/30"}`}
                      />
                    ))}
                  </div>
                </div>
        ...
        <div className="mt-8 text-center">
          <Link href="/reviews" className="inline-flex items-center gap-2 font-medium text-accent hover:underline">
            Read All Reviews
          </Link>
        </div>
      </div>
