...
export default function ReelsPage() {
  const { data: reels } = useActiveReels()
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 pb-20 md:pb-0">
        <div className="container mx-auto px-4 py-6 md:py-10">
          <div className="mb-8">
            <h1 className="mb-2 text-3xl font-bold md:text-4xl">Reels</h1>
            <p className="text-muted-foreground">Watch our latest product reviews and unboxings</p>
          </div>
            ...
            {reels.map((reel) => (
              <button key={reel.id} onClick={() => setSelectedReel(reel)} className="group relative aspect-[9/16] overflow-hidden rounded-2xl bg-secondary text-left">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex size-14 items-center justify-center rounded-full bg-background/20 backdrop-blur-sm transition-transform group-hover:scale-110">
      ...
      {selectedReel ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/95 p-4">
          <div className="relative w-full max-w-lg">
            <Button variant="ghost" size="icon" className="absolute -top-12 right-0 rounded-full text-white hover:bg-white/10" onClick={() => setSelectedReel(null)}>
              <X className="size-6" />
            </Button>
            ...
            <a href={selectedReel.reelUrl} target="_blank" rel="noreferrer" className="block">
              <div className="relative aspect-[9/16] overflow-hidden rounded-3xl bg-secondary">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-foreground/90 to-transparent p-6">
                  <h3 className="text-lg font-bold text-white">{selectedReel.title}</h3>
                  <p className="mt-2 text-sm text-white/70 line-clamp-2">{selectedReel.caption}</p>
                </div>
              </div>
            </a>
          </div>
