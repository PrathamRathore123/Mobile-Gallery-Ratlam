...
type ReelForm = { title: string; reelUrl: string; thumbnail: string; caption: string; sortOrder: string; active: boolean; featured: boolean }

const emptyForm: ReelForm = { title: "", reelUrl: "", thumbnail: "", caption: "", sortOrder: "0", active: true, featured: false }

function toForm(reel: Reel): ReelForm {
  return { title: reel.title, reelUrl: reel.reelUrl, thumbnail: reel.thumbnail, caption: reel.caption, sortOrder: String(reel.sortOrder), active: reel.active, featured: reel.featured }
}
...
export default function AdminReelsPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true)
    const payload = { title: form.title.trim(), reelUrl: form.reelUrl.trim(), thumbnail: form.thumbnail.trim(), caption: form.caption.trim(), sortOrder: Number(form.sortOrder) || 0, active: form.active, featured: form.featured }
    if (editingId) await updateReel(editingId, payload)
    else await createReel(payload)
    setForm(emptyForm)
    setEditingId(null)
    await refresh()
    setSaving(false)
  ...
  const handleEdit = (item: Reel) => { setEditingId(item.id); setForm(toForm(item)) }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit} className="mb-6 rounded-2xl border border-border bg-card p-5">
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <CloudinaryUpload buttonText="Upload Thumbnail" onUploaded={(urls) => setForm((prev) => ({ ...prev, thumbnail: urls[0] ?? prev.thumbnail }))} />
          {form.thumbnail ? <div className="relative h-12 w-12 overflow-hidden rounded-md border border-border"><Image src={form.thumbnail} alt="Thumbnail" fill className="object-cover" /></div> : null}
        </div>

        <div className="mt-4"><label className="mb-2 block text-sm font-medium">Caption</label><textarea className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.caption} onChange={(event) => setForm((prev) => ({ ...prev, caption: event.target.value }))} /></div>
      ...
      <div className="hidden overflow-x-auto rounded-2xl border border-border bg-card md:block">
        <table className="w-full min-w-[640px]">
          ...
          <tbody>
            {loading ? Array.from({ length: 4 }).map((_, i) => (
              <tr key={i} className="border-t border-border"><td className="px-4 py-4"><Skeleton className="h-4 w-40" /></td><td className="px-4 py-4"><Skeleton className="h-4 w-20" /></td><td className="px-4 py-4"><Skeleton className="h-4 w-24" /></td><td className="px-4 py-4"><div className="flex justify-end"><Skeleton className="h-8 w-24" /></div></td></tr>
            )) : null}
            {!loading && data.length === 0 ? <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-muted-foreground">No reels yet.</td></tr> : null}
            {!loading ? data.map((item) => (
              <tr key={item.id} className="border-t border-border text-sm">
                <td className="px-4 py-4 font-medium">{item.title}</td>
                <td className="px-4 py-4"><a href={item.reelUrl} target="_blank" rel="noreferrer" className="text-accent hover:underline">Open link</a></td>
